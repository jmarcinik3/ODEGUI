...
class Model:
    ...
    def getParameters(self, names: Union[str, List[str]] = None) -> Union[Quantity, Dict[str, Quantity]]:
        ...
        """
        if isinstance(names, str): return self.parameters[names]
        elif isinstance(names, Symbol): return self.getParameters(names=str(names))
        elif isinstance(names, list): return {str(name): self.getParameters(names=name) for name in names}
        elif names is None: return self.parameters
        else: raise RecursiveTypeError(names, [str, Symbol])
    def getVariables(self, functions: Union[Function, List[Function]] = None) -> List[Symbol]:
        ...
        if isinstance(functions, Function): return functions.getVariables()
        elif isinstance(functions, list):
            variables = []
            for function in functions: variables.extend(self.getVariables(functions=function))
            return unique(variables)
        else: raise RecursiveTypeError(functions, Function)
    ...
    def loadFunctionsFromFiles(self, function_ymls: Union[str, List[str]]) -> None:
        ...
        for function_yml in function_ymls:
            ...
            for name in file.keys():
                ...
                info_keys = info.keys()
                if "form" in info_keys: function = eval(info["form"])
                elif "pieces" in info_keys: function = [self.getFunctions(names=piece_name) for piece_name in info["pieces"]]
                else: raise ValueError("info from functions_yml file must contain either form or pieces")
    ...
    def saveTimeEvolutionTypesToFile(self, filename: str) -> None:
        ...
        time_evolution_types = {str(derivative.getVariable()): derivative.getTimeEvolutionType() for derivative in derivatives}
        file = open(filename, 'w')
    ...
    def getFunctionNames(self, functions: Union[Function, List[Function]] = None) -> Union[str, List[str]]:
        ...
        """
        if isinstance(functions, Function): return functions.getName()
        elif isinstance(functions, list): return [self.getFunctionNames(functions=function) for function in functions]
        elif functions is None: return self.getFunctionNames(functions=self.functions)
        else: raise RecursiveTypeError(functions, Function)
    def getIndex(self, names: Union[str, List[str]]) -> Union[int, List[int]]:
        ...
        """
        if isinstance(names, (str, list)): return getIndicies(names, self.getFunctionNames(), str)
        else: raise RecursiveTypeError(names)
    def getFunctions(self, names: Union[str, List[str]] = None, filter_type: Type[Function] = None) -> Union[Function, List[Function]]:
        """
        __Purpose__
        ...
        if isinstance(names, str):
            ...
            if filter_type is None or isinstance(function, filter_type): return function
            else: raise TypeError(f"names input must correspond to {filter_type:s}")
        elif isinstance(names, list):
            functions = [self.getFunctions(names=name) for name in names]
            if filter_type is None: return functions
            else: return [function for function in functions if isinstance(function, filter_type)]
        elif names is None: return self.getFunctions(names=self.getFunctionNames(), filter_type=filter_type)
        else: raise RecursiveTypeError(names)
    ...
    def getDerivatives(self, time_evolution_types: Union[str, List[str]] = None) -> List[Derivative]:
        ...
        if isinstance(time_evolution_types, str):
            ...
            return derivatives
        elif time_evolution_types is None: return self.getFunctions(filter_type=Derivative)
        else: raise RecursiveTypeError(time_evolution_types)
    def getEquilibriumSolutions(self, names: Union[str, List[str]] = None,
        ...
        if isinstance(names, (str, list)):
            ...
            if not all([isinstance(function, Derivative) for function in equilibria]): raise TypeError("names must correspond to Derivative stored in Model")
        elif names is None: equilibria = self.getDerivatives(time_evolution_types="Equilibrium")
        else: raise RecursiveTypeError(names)

        equilibrium_count = len(equilibria)
        if equilibrium_count == 0: return {}
    ...
    def getConstantSubstitutions(self, names: Union[str, List[str]] = None) -> Dict[Symbol, Expr]:
        ...
        if isinstance(names, (str, list)):
            ...
            for function in constant_functions:
                if not isinstance(function, Derivative): raise TypeError("names must correspond to Derivative stored in Model")
        elif names is None: constant_functions = self.getDerivatives(time_evolution_types="Constant")
        else: raise RecursiveTypeError(names)
    ...
    def getDerivativeVector(self, names: List[str] = None,
            ...
            lambdified: bool = False) -> Tuple[Union[List[Expr], function], Optional[Dict[Symbol, Expr]]]:
        """
        ...
        else: temporal_derivatives = self.getDerivativesFromVariableNames(names=names)
        
        derivative_vector = []
        for derivative in temporal_derivatives:
            if expanded: form = derivative.getForm(generations="all")
            else: form = derivative.getForm()
        ...
        if substitute_equilibria: return derivative_vector, equilibrium_solutions
        else: return derivative_vector
    ...
    def getInitialValues(self,
        ...
        if isinstance(names, str): return initial_values[Symbol(names)]
        elif isinstance(names, Symbol): return initial_values[names]
        elif isinstance(names, list):
            initial_value = lambda name: self.getInitialValues(names=name, initial_values=initial_values)
            if return_type == list: return [initial_value(name) for name in names]
            elif return_type == ndarray: return np.array([initial_value(name) for name in names])
            elif return_type == dict: return {Symbol(name): initial_value(name) for name in names}
            else: raise ValueError("return_type must be list, ndarray, or dict")
        else: raise RecursiveTypeError(names, [str, Symbol])
    def getDerivativeVariables(self,
        ...
        if return_type == Symbol: return variables
        elif return_type == str: return [str(variable) for variable in variables]
        else: raise ValueError("return_type must be Symbol or str")
    
    def getDerivativesFromVariableNames(self, names: Union[str, Symbol, List[Union[str, Symbol]]]) -> Union[Derivative, List[Derivative]]:
        """
        ...
        elif isinstance(names, list): return [self.getDerivativesFromVariableNames(names=name) for name in names]
        else: RecursiveTypeError(names, [str, Symbol])
...
class Parent:
    ...
    def setAsParent(self, children: Union[Function, List[Function]]) -> None:
        ...
        """
        if isinstance(children, Function): children.addParent(self)
        elif isinstance(children, list):
            for child in children: self.setAsParent(child)
        else: raise RecursiveTypeError(children, Function)
    ...
    def getChildIndex(self, names: Union[str, List[str]]) -> Union[int, List[int]]:
        ...
        """
        if isinstance(names, (str, list)): return getIndicies(names, self.getChildrenNames(), str)
        else: raise RecursiveTypeError(names)
    def getChildren(self, names: Union[str, List[str]] = None) -> Union[Function, List[Function]]:
        ...
        :param names: name(s) of child(s) to retrieve from parent
        """
        if isinstance(names, str): return self.children[self.getChildIndex(names)]
        elif isinstance(names, list): return [self.getChildren(names=name) for name in names]
        elif names is None: return self.getChildren(names=self.getChildrenNames())
        else: raise RecursiveTypeError(names)
    def getChildrenNames(self, functions: Union[Function, List[Function]] = None) -> Union[str, List[str]]:
        ...
        """
        if isinstance(functions, Function): return functions.getName()
        elif isinstance(functions, list): return [self.getChildrenNames(functions=function) for function in functions]
        elif functions is None: return self.getChildrenNames(functions=self.children)
        else: raise RecursiveTypeError(functions, Function)
    ...
    def getInstanceArguments(self, name: str, specie: str = None) -> Union[Symbol, List[Symbol]]:
        ...
        """
        if isinstance(specie, str): return self.instance_arguments[name][specie]
        elif specie is None: return self.instance_arguments[name]
        else: raise TypeError("specie must be str")
...
class Child:
    ...
    def addParent(self, parents: Union[Function, List[Function]]) -> None:
        ...
        """
        if isinstance(parents, Function) and parents not in self.getParents(): self.parents.append(parents)
        elif isinstance(parents, list):
            for parent in parents: self.addParent(parent)
        else: raise RecursiveTypeError(parents, Function)
    ...
    def getParentIndex(self, names: Union[str, List[str]]) -> Union[int, List[int]]:
        ...
        """
        if isinstance(names, (str, list)): return getIndicies(names, self.getParentNames(), str)
        else: raise RecursiveTypeError(names)
    def getParents(self, names: Union[str, List[str]] = None) -> Union[Function, List[Function]]:
        ...
        """
        if isinstance(names, str): return self.parents[self.getParentIndex(names)]
        elif isinstance(names, list): return [self.getParents(names=name) for name in names]
        elif names is None: return self.getParents(names=self.getParentNames())
        else: raise RecursiveTypeError(names)
    def getParentNames(self, functions: Union[Function, List[Function]] = None) -> Union[str, List[str]]:
        ...
        """
        if isinstance(functions, Function): return functions.getName()
        elif isinstance(functions, list): return [self.getParentNames(functions=function) for function in functions]
        elif functions is None: return self.getParentNames(functions=self.parents)
        else: raise RecursiveTypeError(functions, Function)
...
class Function(Child, Parent):
    ...
    def addVariables(self, variables: Union[Symbol, List[Symbol]]) -> None:
        ...
        """
        if isinstance(variables, Symbol): self.variables.append(variables)
        elif isinstance(variables, list):
            for variable in variables:
                if not isinstance(variable, Symbol): raise TypeError("variable must be sympy.Symbol")
                self.addVariables(variable)
        elif variables is None: pass
        else: raise RecursiveTypeError(variables, Symbol)
    def addParameters(self, parameters: Union[Symbol, List[Symbol]]) -> None:
        ...
        """
        if isinstance(parameters, Symbol): self.parameters.append(parameters)
        elif isinstance(parameters, list):
            for parameter in parameters:
                if not isinstance(parameter, Symbol): raise TypeError("parameter must be sympy.Symbol")
                self.addParameters(parameter)
        elif parameters is None: pass
        else: raise RecursiveTypeError(parameters, Symbol)
    ...
    def setModel(self, model: Model) -> None:
        ...
        else: raise TypeError("model input must be Model")
...
class Derivative:
    ...
    def getVariable(self, return_type: Type[Symbol, str] = Symbol) -> Union[Symbol, str]:
        ...
        """
        if return_type == Symbol: return self.variable
        elif return_type == str: return str(self.variable)
        else: raise ValueError("return_type must be sp.Symbol or str")
    def setVariable(self, variable: Union[str, Symbol]) -> None:
        ...
        """
        if isinstance(variable, str): self.setVariable(Symbol(variable))
        elif isinstance(variable, Symbol): self.variable = variable
        else: raise TypeError("variable input must be str of sympy.Symbol")
    ...
    def getInitialCondition(self) -> Union[str, float]:
        ...
        """
        if self.getTimeEvolutionType() == "Equilibrium": initial_condition = "Equilibrium"
        else: initial_condition = self.initial_condition
...
class Dependent:
    ...
    def setGeneralArguments(self, arguments: Union[Symbol, List[Symbol]], species: str = None) -> None:
        ...
        if isinstance(species, str):
            if isinstance(arguments, Symbol): self.general_arguments[species] = [arguments]
            ...
            elif arguments is None: self.general_arguments[species] = []
            else: raise RecursiveTypeError(arguments, Symbol)
        elif species is None:
            if isinstance(arguments, dict):
                for specie, argument in arguments.items(): self.setGeneralArguments(argument, specie)
            else: raise TypeError("arguments input must be dict when species input is None")
        else: raise TypeError("species input must be str")
    ...
    def getGeneralSpecies(self, nested: bool = True) -> List[str]:
        ...
        if nested:
            for child in self.getChildren():
                ...
                if isinstance(child_species, str): species.append(child_species)
                elif isinstance(child_species, list): species.extend(child_species)
                elif isinstance(child_species, KeysView): species.extend(list(child_species))
                else: raise TypeError(f"species for {child.getName():s} must be str, list, or dict_keys")
        return unique(species)
    def getGeneralArguments(self, species: Union[str, List[str]] = None, nested: bool = False) -> Union[List[Symbol], Dict[str, List[Symbol]]]:
        """
        __Purpose__
        ...
        if nested:
            ...
            for child in self.getChildren():
                ...
                if isinstance(child_general_arguments, Symbol): general_arguments.append(child_general_arguments)
                elif isinstance(child_general_arguments, list): general_arguments.extend(child_general_arguments)
                else: raise RecursiveTypeError(child_general_arguments)
            return general_arguments
        elif not nested:
            if isinstance(species, str):
                general_species = self.getGeneralSpecies(nested=False)
                if species in general_species: return self.general_arguments[species]
                elif species not in general_species: return []
            elif isinstance(species, list): return {specie: self.getGeneralArguments(specie) for specie in species}
            elif species is None: return {specie: self.getGeneralArguments(specie) for specie in
                self.getGeneralSpecies()}
            else: raise RecursiveTypeError(species)
    ...
    def getInstanceArgumentSubstitutions(self, parent: Function, specie: str) -> List[Expr]:
        ...
        if specie == "functions":
            ...
            for instance_name in instance_names:
                ...
                if isinstance(sibling, Dependent): instance_functions.append(sibling.getInstanceArgumentFunction(parent))
                elif isinstance(sibling, Function): instance_functions.append(sibling.getForm(generations="all"))
                else: raise TypeError(f"sibling for {self_name:s} must be Function")
            return instance_functions
        else: return parent.getInstanceArguments(specie=specie, name=self_name)
...
class Piecewise:
    ...
    def getForm(self, generations: Union[int, str] = 1) -> spPiecewise:
        ...
        if generations == "all": pieces = [function.getForm(generations="all") for function in functions]
        elif generations >= 0: pieces = [function.getForm(generations=generations) for function in functions]
        else: raise ValueError("generations must be 'all' or some integer greater than or equal to 0")
    ...
    def getVariables(self, nested: bool = True) -> List[Symbol]:
        ...
        return unique(variables)
    def getParameters(self, nested: bool = True, return_type: Type[Symbol, str] = Symbol) -> Union[List[Symbol], List[str]]:
        """
        ...
        if return_type == Symbol: return unique_parameters
        elif return_type == str: return [str(parameter) for parameter in unique_parameters]
        else: raise ValueError("return_type must be Symbol or str")
...
class NonPiecewise:
    ...
    def getForm(self, parent: Function = None, substitute_dependents: bool = True, generations: Union[int, str] = 0) -> Expr:
        """
        __Purpose__
        ...
        for child in self.getChildren():
            ...
            if isinstance(child, Dependent):
                if substitute_dependents: child_expression = child.getForm(parent=self)
                else: child_expression = child_symbol
            elif isinstance(child, Independent):
                if generations == "all": child_expression = child.getForm(substitute_dependents=substitute_dependents, generations=generations)
                elif generations >= 1: child_expression = child.getForm(substitute_dependents=substitute_dependents, generations=generations - 1)
                elif generations == 0: child_expression = child_symbol
                else: raise ValueError("generations must be 'all' or some integer greater than or equal to 0")
            else: raise TypeError("child must be of type Dependent xor Independent")
    ...
    def getVariables(self, nested: bool = True) -> List[Symbol]:
        ...
        if nested:
            for child in self.getChildren():
                ...
                if isinstance(child_variables, Symbol): variables.append(child_variables)
                elif isinstance(child_variables, list): variables.extend(child_variables)
                else: raise RecursiveTypeError(child_variables, Symbol)
        return unique(variables)
    def getParameters(self, nested: bool = True, return_type: Type[Symbol, str] = Symbol) -> Union[List[Symbol], List[str]]:
        """
        Get unique list of parameters in function.
        ...
        if nested:
            for child in self.getChildren():
                ...
                if isinstance(child_parameters, Symbol): parameters.append(child_parameters)
                elif isinstance(child_parameters, list): parameters.extend(child_parameters)
                else: raise RecursiveTypeError(child_parameters, Symbol)
        unique_parameters = unique(parameters)
...
def FunctionMaster(name: str, function: Union[Expr, List[Function]],
    ...
    class FunctionCore(Function, *inheritance):
        ...
        def __init__(self, name: str, function: Union[Expr, List[Function]],
            ...
            if Dependent in inheritance: Dependent.__init__(self, **kwargs["Dependent"])
            elif Independent in inheritance: Independent.__init__(self)
            else: raise ValueError("Function must inherit either Dependent or Independent")
            
            if Piecewise in inheritance: Piecewise.__init__(self, function, **kwargs["Piecewise"])
            elif NonPiecewise in inheritance: NonPiecewise.__init__(self, function)
            else: raise ValueError("Function must inherit either Piecewise of NonPiecewise")
    
    return FunctionCore(name, function, inheritance, **kwargs)

def getFunctionInfo(info: Dict[str, Union[str, List[str], Dict[str, Union[str, List[str]]]]], model: Model = None) -> Dict[str, List[str]]:
    """
    __Purpose__
    ...
    def getChildren(model: Model, info: dict) -> Dict[str, Dict[str, Union[Function, Union[Symbol, List[Symbol]]]]]:
        ...
        for child_name in children_names:
            ...
            if child_info is not None: children_dict[child_name] = {argument_type: var(child_info[argument_type]) for
                argument_type in child_info.keys()}
            elif child_info is None: children_dict[child_name] = {}
...
def createFunction(name: str, function: Union[str, List[str]], properties: Tuple[str] = (), **kwargs) -> None:
    ...
    if "Dependent" in properties: inheritance.append(Dependent)
    elif "Independent" in properties: inheritance.append(Independent)
    else: raise ValueError("Function must have either 'Dependent' or 'Independent' as property")
    
    if "Piecewise" in properties: inheritance.append(Piecewise)
    elif "Piecewise" not in properties: inheritance.append(NonPiecewise)
