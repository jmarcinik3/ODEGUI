...
def readVar2Tex(filepath: str = var2tex_filepath) -> dict:
...
def readLayout(filepath: str) -> dict:
    """
    Read config file containing layout for window.
...
def getDimensions(
    keys: List[str],
    filepath: str = layout_dimensions_filepath
...
def loadConfig(
    ...
    :param filepath: path of file to load contents from.
        Must be relative to *.zip file if :paramref:`~Config.loadConfig.archive` is given.
