...
class Row:
    def __init__(self, name: str = None, elements: Union[sg.Element, List[sg.Element]] = None, window: Window = None,
            dimensions: Dict[Tuple[Optional[float], Optional[float]]] = None) -> None:
        self.name = name
        self.window = window
        self.dimensions = dimensions
        
        if isinstance(elements, sg.Element): self.elements = [elements]
        elif isinstance(elements, list): self.elements = elements
        elif elements is None: self.elements = []
        else: raise TypeError("elements must be sg.Element or list")
    
    def getName(self) -> str: return self.name
    
    def addElements(self, elements: Union[sg.Element, List[sg.Element]]) -> None:
        if isinstance(elements, sg.Element): self.elements.append(elements)
        elif isinstance(elements, list):
            for element in elements: self.addElements(element)
        else: raise TypeError("elements must be sg.Element or list")
    def getRow(self) -> List[sg.Element]: return self.elements
    def getLayout(self) -> List[List[sg.Element]]: return [self.elements]
...
class Layout:
    def __init__(self, rows: Union[Row, List[Row]] = None) -> None:
        if isinstance(rows, Row): self.rows = [rows]
        elif isinstance(rows, list): self.rows = rows
        elif rows is None: self.rows = []
        else: raise TypeError("rows must be sg.Element or list")
    
    def addRows(self, rows: Union[Row, List[Row]]) -> None:
        if isinstance(rows, Row): self.rows.append(rows)
        elif isinstance(rows, list):
            for row in rows: self.addRows(row)
        else: raise TypeError("rows must be Row or list")
    def getRows(self) -> List[Row]: return self.rows
    def getLayout(self) -> List[List[sg.Element]]:
        layout = [[]]
        for row in self.getRows(): layout += row.getLayout()
        return layout
...
class Tab:
    ...
    def getDimensions(self, **kwargs) -> Union[Dict[str, Tuple[Optional[float], Optional[float]]], Tuple[Optional[float], Optional[float]]]:
        return self.getWindowObject().getDimensions(**kwargs)
...
class TabGroup:
    def __init__(self, tabs: Union[Tab, List[Tab]], name: str = None, suffix_layout: Layout = None) -> None:
        ...
        if isinstance(tabs, (Tab, sg.Tab)): self.tabs = [tabs]
        elif isinstance(tabs, list): self.tabs = tabs
        elif tabs is None: self.tabs = []
        else: raise TypeError("tabs must be Tab or list")
    
    def getName(self) -> str: return self.name
    def getTabs(self) -> List[Tab]: return self.tabs
    
    def getTabGroup(self) -> sg.TabGroup:
        tabs = []
        for tab in self.getTabs():
            if isinstance(tab, Tab): tabs.append(tab.getTab())
            elif isinstance(tab, sg.Tab): tabs.append(tab)
            else: raise TypeError("tab must be Tab or sg.Tab")
        return sg.TabGroup([tabs])
    def getSuffixLayout(self) -> List[List[sg.Element]]: return self.suffix_layout.getLayout()
...
class Window:
    def __init__(self, name: str, runner: WindowRunner,
            dimensions: Dict[str, Tuple[Optional[float], Optional[float]]] = None) -> None:
        self.name = name
        self.runner = runner
        self.dimensions = dimensions
    ...
    def getName(self) -> str: return self.name
    def getWindowRunner(self) -> WindowRunner: return self.runner
    def getDimensions(self, name: str = None) -> Union[Dict[str, Tuple[Optional[float], Optional[float]]], Tuple[Optional[float], Optional[float]]]:
        if isinstance(name, str):
            dimensions = self.getDimensions()
            return dimensions[name]
        elif name is None: return self.dimensions
        else: raise TypeError("column must be str")
    ...
    def getKey(self, prefix: str, tag: str = None) -> str: return self.key_list.getKey(prefix, tag)
    def getKeyList(self, prefixes: str = None) -> List[str]: return self.key_list.getKeyList(prefixes)
...
class TabbedWindow(Window):
    def __init__(self, name: str, runner: WindowRunner,
            dimensions: Dict[str, Tuple[Optional[int], Optional[int]]] = None) -> None:
        super().__init__(name, runner, dimensions=dimensions)
        self.tabs = []
    
    def addTabs(self, tabs):
        if isinstance(tabs, (Tab, sg.Tab)): self.tabs.append(tabs)
        elif isinstance(tabs, list):
            for tab in tabs: self.addTabs(tab)
        else: raise TypeError("tabs must be Tab, sg.Tab, or list")
...
class WindowRunner:
    ...
    def getName(self) -> str: return self.name
    def getWindow(self) -> sg.Window:
        if self.window is None: self.window = self.getWindowObject().getWindow()
        return self.window
    ...
    def getKey(self, prefix: str, tag: str = None) -> str: return self.getWindowObject().getKey(prefix, tag)
    def getKeyList(self, prefixes: str = None) -> List[str]: return self.getWindowObject().getKeyList(prefixes)
    ...
    def getElements(self, keys: Union[str, List[str]]) -> Union[sg.Element, List[sg.Element]]:
        ...
        """
        if isinstance(keys, str): return self.getWindow()[keys]
        elif isinstance(keys, list): return [self.getElements(keys=key) for key in keys]
        else: raise TypeError("keys must be str or list")
