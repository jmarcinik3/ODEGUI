...
class ChooseParametersWindow(Window):
    def __init__(self, name: str, runner: ChooseParametersWindowRunner, quantities: Dict[str, Quantity] = None, filename: str = ''):
        dimensions = {
...
class ChooseParametersWindowRunner(WindowRunner):
    ...
    def getChosenParameters(self) -> List[str]:
        ...
        if event in [sg.WIN_CLOSED, "Cancel"]: return []
        elif event == "Submit": return [name for name in parameters if parameters[name]]
